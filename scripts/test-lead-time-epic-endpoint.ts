...
const API_URL = process.env.API_URL || 'http://localhost:3000/api/dora/lead-time-epic';
...
async function main() {
  ...
  if (res.ok && data.meanLeadTimeDays !== undefined) {
    console.log('✅ Test endpoint lead-time-epic: SUCCESS');
    console.log(`Mean Lead Time (Days): ${data.meanLeadTimeDays}`);
    console.log(`Mean Lead Time (READY_FOR_UAT): ${data.meanLeadTimeReadyForUAT}`);
    console.log(`Total Epics: ${data.epics.length + data.skippedEpics.length}`);
  } else {
    console.error('❌ Test endpoint lead-time-epic: ERROR');
  }
}
