...
export async function getGoogleCalendarClient(userId: string) {
  ...
  if (!settings?.googleAccessToken) {
    throw new Error('Google Calendar not connected. Please connect your Google account in Settings.');
  }
...
export async function getUncachableGoogleCalendarClient() {
  throw new Error('This function is deprecated. Use getGoogleCalendarClient(userId) instead.');
}
